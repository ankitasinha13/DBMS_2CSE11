...
~~~sql
SELECT MAX(SAL) AS MAX_SAL_DEPTNO_20
FROM EMPLOYEE
WHERE DEPTNO = 20;
~~~
...
~~~sql
SELECT SUM(SAL) AS TOTAL_ANALYST_DEPTNO_40_SAL
FROM EMPLOYEE
WHERE JOB = 'ANALYST'
AND DEPTNO = '40';
~~~
...
~~~sql
SELECT UPPER(ENAME) AS EMP_UPPER_NAME
FROM EMPLOYEE;
~~~

### 12.Display the names of the employee in Lowercase.
~~~sql
SELECT LOWER(ENAME) AS EMP_LOWER_NAME
FROM EMPLOYEE;
~~~
...
~~~sql
SELECT LENGTH('ANKITA SINHA') AS NAME_LENGTH;
~~~
